...
bool Tags::has(std::string tagToSearch) {
	if (str_is_blank(tagToSearch)) return false;

	if (str_has_value(tagToSearch)) tagToSearch = tagToSearch.substr(0, tagToSearch.find('='));
	if (str_is_blank(tagToSearch)) return false; 
...
void Tags::setTags(std::string tagsStr) {
	tags.clear();
	if (str_is_blank(tagsStr)) return;
