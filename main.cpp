...
void printHelp() {
	...
	std::cout << "\t-L / --ignore-label <string>\tIgnore Labels, separated by ','" << std::endl;
...
void printToday() {
	...
	std::cout << today.getDayName() << " " << today.toString() << std::endl;
}
...
struct {
	...
	std::list<std::string> labelsToIgnore = {};
} options;

//returns `stoi(str)` or printHelp if is not parseable
int_least16_t toInt(std::string str) {
	for (char c : str) if (!isdigit(c)) printHelp();
	return std::stoi(str);
}

//Must have set the `$EDITOR` env
void editFiles() {
...
void parseArguments(int argc, std::vector<std::string> args) {
	for (int i = 0; i < argc; i++) {
		...
		if (arg == "-L" || arg == "--ignore-label") options.labelsToIgnore = str::split(",", argNext);
	}
}
...
int main (int argc, char *argv[]) {
	...
	//Query Events
	for (Date date = today - options.daysBefore; date != today + options.daysAfter; date = date + 1) {
		for (Event* event : eventList) {
			if (options.labelsToSearch.empty()) {
				if (event->isValidIn(&date)) {
					if (date >= today && !printedTodayBanner) {
				...
				for (std::string label : options.labelsToSearch) {
					if (event->hasLabel(label) && event->isValidIn(&date)) {
						if (date >= today && !printedTodayBanner) {
							printedTodayBanner = true;
						}
						event->print(&date);
